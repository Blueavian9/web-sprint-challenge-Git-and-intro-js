...
function listOfNames(artists){
  let copyArr = [...artists];
  return copyArr.map((element) => element.name);
}
...
function lotsOfArt(artists) {
  ...
  for (let index = 0; index < artists.length; index++) {
    if (artists[index].paintings > 100) {
      moreThen100.push(artists[index].name);
    }
  }  
  return moreThen100;
}
...
function artistByCountry(artists, nationality){
  ...
  for (const index in artists) {
    if (artists[index].nationality === nationality) {
      nameArr.push(artists[index].name);
    }
  }  
  return nameArr;
}
